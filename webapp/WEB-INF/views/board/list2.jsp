				...
				<div id="list">
					...
					<div id="paging">
						<ul>
							...
							<li><a href="${pageContext.request.contextPath }/board/list2?crtPage=${listMap.startPageBtnNo-1 }&keyword=${listMap.keyword}">◀</a></li>
							</c:if>
							<c:forEach var="i" begin="${listMap.startPageBtnNo }" end="${listMap.endPageBtnNo }" step="1">
								<c:choose>
									<c:when test="${param.crtPage == i }">
										<li class="active"><a href="${pageContext.request.contextPath }/board/list2?crtPage=${i }&keyword=${listMap.keyword}">${i }</a></li>
									</c:when>
									<c:otherwise>
										<li><a href="${pageContext.request.contextPath }/board/list2?crtPage=${i }&keyword=${listMap.keyword}">${i }</a></li>
									</c:otherwise>
								</c:choose>
							</c:forEach>
							<c:if test="${listMap.next == true }">
							<li><a href="${pageContext.request.contextPath }/board/list2?crtPage=${listMap.endPageBtnNo+1 }&keyword=${listMap.keyword}">▶</a></li>
							</c:if>
						</ul>
